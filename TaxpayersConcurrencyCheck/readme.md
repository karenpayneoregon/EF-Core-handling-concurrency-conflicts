...
:pushpin:  [Serilog](https://serilog.net/) NuGet package is used to log what goes on under the covers which will help to better understand how dealing with concurrency conflicts works and what information is available to a developer.
...
1. `entry.OriginalValues.SetValues(databaseValues!)` sets the local values to when looping back to `await Context.SaveChangesAsync();`
